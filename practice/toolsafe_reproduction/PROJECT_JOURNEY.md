...
这一阶段最初计划直接实现可审计的`action_aware`处置消融；2026-09-15复核后将该实现后移，
先执行完整Guard能力审计。原因是单条误拦同时包含Think、MUR字段和Harmfulness之间的语义
张力，不能先用策略层把模型错误隐藏起来。Guard原始三字段及Think必须完整保留，不能通过
修改日志或标签掩盖误判。

### 16.13 当前节点：完整Guard能力审计前置（2026-09-15）

当前已完成而且可核验的端到端规模仍然有限：正式API Agent共有2条Guard-in-loop
`ts_flow`轨迹和3条无Guard的`react`参考轨迹；只有workspace
`user_task_13 + tool_knowledge + injection_task_0`形成同一identity的配对对照。该结果
用于证明真实工具、Guard、Token FSM、阻断和反馈链路，而不能作为总体攻击成功率或utility
提升结论。离线官方7B Guard已评测87条banking和146条clean validation，共233条；ASB
继续冻结，使用量为0。

代码复核还纠正了对`strict`的简化描述。运行时不是直接执行`Harmfulness>=0.5`，而是沿用
官方聚合器：Being Attacked和MUR各贡献0.4，H=0.5贡献0.1，H=1.0贡献0.2；总分大于0.5
映射1.0、等于0.5映射0.5、其余映射0.0，SecReAct再阻断`risk>=0.5`。这解释了
`yes/no/0.5`为何被阻断，也暴露`no/no/1.0`会被聚合为0.0的潜在漏放组合。聚合器问题与
Guard模型能力必须分开评估。

下一步不启动训练、不实现Action-aware，也不更换Agent来制造攻击成功样本。先在不读取ASB
的前提下完成Guard能力审计：233条留出/外部样本作为泛化报告，1656条裁决语料按split和
来源仅作诊断；报告格式、Think、三个字段、三字段完全一致、Think-字段语义一致性、字段到
聚合风险的映射，以及高危漏报和合法误报。若错误集中于模型语义，先做数据复核和低风险
LoRA；若字段可靠而聚合/执行错误，才实现`strict`与`action_aware`处置消融；若两者同时
存在，则分别修复并独立评测。
